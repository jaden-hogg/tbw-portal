...
- **Hardcoded exclusion** (`_EXCLUDED_WEEKS`): weeks that never appear as an invoice at all, even if orders still map to them — currently just 7/03, whose only content was 2 orders misassigned by the pre-fix ship-date bucketing bug (the real ship date correctly lands them on 7/10 instead). Add to that set for a similar one-off removal.
