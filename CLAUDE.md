...
Silently skips (no exception, no blocked order) if `PRODUCTION_PORTAL_URL`/`PRODUCTION_INGEST_TOKEN` aren't set — same "missing creds = no-op" convention as the Gmail notification just above. Auth is `X-Production-Token`, a token distinct from any other credential in this app. The push does check `resp.raise_for_status()` (2026-07-22) so a bad URL/token surfaces as a real logged exception — see NOTES.md's misconfiguration incident.
