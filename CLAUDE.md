...
- When on, the normal 15oz/11oz qty fields are replaced with four fields — 11oz Mug Qty, 11oz Box Qty, 15oz Mug Qty, 15oz Box Qty — plus a manual **Ship To Address** textarea and an optional **Line Items** textarea. The Choose Files button still attaches whatever reference files/photos are relevant.
...
- **Address auto-fill**: on blur of PO Number (or when the toggle is switched on with a PO already entered), the client calls `GET /lookup_replacement/<po_number>`, which looks up the *original* `TBW-<po_number>` order in ShipStation (exact match, not the `-REPLACEMENT` one) and pre-fills the address textarea from its `shipTo`. Only fills if the textarea is still empty, so it never clobbers a manual edit — used when the replacement ships to the same place as the original order.
- **Box label expansion for replacements**: since there's no PO PDF to parse designs/quantities from, the **Line Items** field takes manually-typed designs, one per line (`"<qty> x <description>"`, parsed by `parse_manual_line_items` in `box_labels.py`). If a file with "box label" in its name is attached, `submit_order` runs the same `expand_box_labels` vision-matching pipeline as regular orders, sourcing `line_items` from this manual list instead of `parse_po_line_items`. If a box label is attached with no line items typed, expansion fails gracefully and a warning lands in the ShipStation notes (original file kept, unexpanded).
